// Southwood Project Tracker — Full: Risk Tints + Sort + Inline Edit + AI + Aging + Contacts + ICS
// Works with React/ReactDOM + Babel + Tailwind (CDN). No external libraries.
...
function includesWord(hay, needle){ const s=` ${String(hay).toLowerCase()} `; const n=` ${String(needle).toLowerCase()} `; return s.includes(n); }
...
function makeICS({ summary, description, dateISO }) {
  ...
  // For all-day events, DTEND is exclusive; add 1 day:
...
function getNextUpcoming(project){
  ...
  for(const ph of PHASES){
    ...
    if(d){ const t=new Date(d).setHours(0,0,0,0); if(t>=t0) future.push({phase:ph, date:d}); }
  }
...
const isStalled = (p)=> { const age=ageInCurrentPhase(p); if(age===null) return false; const lim=STALL_THRESHOLDS[p.phase]??14; return age>lim; };

// Priority mixing schedule + value
...
function nextFollowUpDate(p){
  const cadence = Number(p.cadenceDays || 14);
  if (!p.lastContact) return null;
  const d = new Date(p.lastContact); d.setHours(0,0,0,0);
  d.setDate(d.getDate()+cadence);
  return d;
}
function followUpStatus(p){
  const cadence = Number(p.cadenceDays || 14);
  const last = p.lastContact? new Date(p.lastContact) : null;
...
function addMilestoneICS(p){
  const next = getNextUpcoming(p);
  if (!next) { alert("This project has no upcoming milestone."); return; }
  const ics = makeICS({
    summary: `${p.id} — ${p.name}: ${next.phase}`,
    description: `${p.client} • ${p.location}`,
    dateISO: next.date
  });
...
function addFollowUpICS(p){
  const next = nextFollowUpDate(p);
  const dateISO = next ? next.toISOString().slice(0,10) : todayStart().toISOString().slice(0,10);
  const label = next ? "Follow-up" : "Initial follow-up";
  const ics = makeICS({
    summary: `${p.id} — ${p.name}: ${label}`,
    description: `Cadence ${p.cadenceDays || 14}d • Contact ${p.contactPerson || ""} ${p.contactEmail || ""}`,
    dateISO
  });
...
function extractDateRange(q){
  ...
  if(includesWord(lower,"this week")){
    const d=new Date(now); const day=d.getDay(); const mon=new Date(d); mon.setDate(d.getDate()-((day+6)%7)); const sun=new Date(mon); sun.setDate(mon.getDate()+7);
    return { start: mon, end: sun };
  }
  if(includesWord(lower,"next week")){
    const d=new Date(now); const day=d.getDay(); const mon=new Date(d); mon.setDate(d.getDate()+(7-((day+6)%7))); const sun=new Date(mon); sun.setDate(mon.getDate()+7);
    return { start: mon, end: sun };
  }
  ...
  if(lower.includes("between ")&&lower.includes(" and ")){
    const si=lower.indexOf("between ")+8; const ai=lower.indexOf(" and ", si);
    if(ai>-1){ const a=String(q).slice(si, ai).trim(); const b=String(q).slice(ai+5).trim(); const da=tryParseDate(a); const db=tryParseDate(b); if(da&&db) return { start:da, end:db }; }
  }
...
function applyAIQuery(projects, query){
  ...
  // overdue keyword (optionally per phase)
  if (includesWord(lower, "overdue")) {
    list = list.filter(p => {
      if (!mentioned.length) return hasOverdue(p);
      return mentioned.some(ph => {
        const d=p.milestones[ph]; return d ? new Date(d).setHours(0,0,0,0) < todayStart().getTime() : false;
      });
    });
  }
  ...
  if (range){
    ...
    list = list.filter(p => {
      ...
      return phases.some(ph => {
        const d = p.milestones[ph]; if (!d) return false;
  ...
  if (includesWord(lower, "focus") || includesWord(lower, "right away") || includesWord(lower, "urgent")){
    ...
    list = list.filter(p => hasOverdue(p) || (getNextUpcoming(p) && new Date(getNextUpcoming(p).date).getTime() <= soon));
  ...
  if (includesWord(lower, "follow-ups due") || includesWord(lower,"overdue follow-ups") || includesWord(lower,"follow up due")){
    list = list.filter(p => followUpStatus(p).due);
  }
  const notContacted = lower.match(/not contacted in (\d+)\s*days?/);
  if (notContacted){ const n=Number(notContacted[1]); list = list.filter(p => !p.lastContact || daysSince(p.lastContact)>=n); }
  ...
  if (includesWord(lower,"contacted this week")){
    const now = todayStart(); const day = now.getDay();
...
const PROJECTS = [
  {
    ...
    value:185000,
    phase:"Manufacturing",
    milestones:{ Design:"2025-08-22", Estimating:"2025-08-25", Permitting:"2025-09-01", Surveying:"2025-09-05", Manufacturing:"2025-09-18", Installing:"2025-09-20" },
    phaseSince:"2025-08-10",
  ...
  },
  {
    ...
    value:92000,
    phase:"Surveying",
    milestones:{ Design:"2025-08-26", Estimating:"2025-08-28", Permitting:"2025-09-10", Surveying:"2025-09-12", Manufacturing:"2025-09-25", Installing:"2025-10-05" },
  ...
  },
  {
    ...
    value:410000,
    phase:"Permitting",
    milestones:{ Design:"2025-08-24", Estimating:"2025-08-27", Permitting:"2025-09-02", Surveying:"2025-09-06", Manufacturing:"2025-09-20", Installing:"2025-10-01" },
  ...
  },
  {
    ...
    value:145000,
    phase:"Estimating",
    milestones:{ Design:"2025-08-25", Estimating:"2025-09-03", Permitting:"2025-09-10", Surveying:"2025-09-14", Manufacturing:"2025-09-28", Installing:"2025-10-03" },
  ...
  },
  {
    ...
    value:56000,
    phase:"Design",
    milestones:{ Design:"2025-09-01", Estimating:"2025-09-05", Permitting:"2025-09-12", Surveying:"2025-09-16", Manufacturing:"2025-09-30", Installing:"2025-10-04" },
  ...
  },
  {
    ...
    value:78000,
    phase:"Installing",
    milestones:{ Design:"2025-07-28", Estimating:"2025-07-31", Permitting:"2025-08-02", Surveying:"2025-08-05", Manufacturing:"2025-08-07", Installing:"2025-08-08" },
...
// =====================================
function App(){
  // load / persist (new key v6)
  const [projects,setProjects] = React.useState(()=>{
    try{ const raw=localStorage.getItem("southwood_projects_v6"); return raw?JSON.parse(raw):PROJECTS; }catch{ return PROJECTS; }
  });
  const [quotes] = React.useState(QUOTES);
  React.useEffect(()=>{ try{ localStorage.setItem("southwood_projects_v6", JSON.stringify(projects)); }catch{} },[projects]);
  ...
  function runClear(){
    setPhasesFilter([...PHASES]); setMinVal(""); setMaxVal("");
    setAiQuery(""); setAiAnswer(""); setSearch("");
  ...
  function emailContact(p){
    ...
    const body = [
      `Hi ${p.contactPerson || "there"},`,
      "",
      `Quick update on ${p.name} (${p.id}).`,
      next ? `• Next due: ${next.phase} on ${new Date(next.date).toLocaleDateString()}` : "• No upcoming milestones on the schedule",
      od.length ? `• Overdue phases: ${od.join(", ")}` : "",
      "",
  ...
  const filtered = React.useMemo(()=>{
    ...
    if (search.trim()){
      ...
      list = list.filter(p =>
        [p.id, p.name, p.client, p.location, p.contactPerson, p.contactEmail]
  ...
  const kpi = React.useMemo(()=>{
    ...
    const dueSoon = projects.filter(p =>
      PHASES.some(ph=>{
        const d=p.milestones[ph]; if(!d) return false;
        const t=new Date(d).getTime(); return t>=now && t<=in30;
      })
    ).length;
    ...
    return { active:projects.length, pipeline, dueSoon, atRisk, totalValue, avgAge, stalled, followDue };
  }, [projects, quotes]);
  ...
  return (
    <div className="min-h-screen bg-[#0b1020] text-zinc-100">
        ...
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              placeholder="Search by ID, client, location, contact…"
      ...
      <main className="max-w-7xl mx-auto px-4 py-6 grid lg:grid-cols-3 gap-6">
        ...
        <section className="lg:col-span-2 space-y-6">
          ...
          <div className="grid sm:grid-cols-2 xl:grid-cols-5 gap-4">
            <Card><CardContent className="p-5"><p className="text-xs uppercase tracking-widest text-zinc-400">Projects</p><p className="text-3xl mt-2 font-semibold">{kpi.active}</p><p className="text-xs text-zinc-400 mt-1">Total value {currency(kpi.totalValue)}</p></CardContent></Card>
            ...
            <Card><CardContent className="p-5"><p className="text-xs uppercase tracking-widest text-zinc-400">Overdue</p><p className="text-3xl mt-2 font-semibold">{kpi.atRisk}</p><p className="text-xs text-zinc-400 mt-1">Projects with any overdue phase</p></CardContent></Card>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        placeholder="Try: follow-ups due, not contacted in 14 days, contacted this week, stalled, in phase over 14 days, due next 7 days, highest value projects"
                  ...
                  <div className="overflow-auto border border-zinc-800 rounded-xl">
                    <table className="min-w-full text-sm">
                      ...
                      <tbody>
                            ...
                            <React.Fragment key={p.id}>
                              <tr className={`border-t border-zinc-800 text-zinc-200 align-top ${risk.row}`}>
                                ...
                                <td className="py-3 px-3 whitespace-nowrap">
                                  ...
                                  <button onClick={()=>addFollowUpICS(p)} className="mt-1 text-xs underline text-emerald-300 hover:text-emerald-200">Add reminder</button>
                                </td>
                        ...
                        {filtered.length===0 && (
                          <tr><td colSpan={9} className="py-6 text-center text-zinc-400">No projects match current filters.</td></tr>
                        )}
                      </tbody>
                    </table>
                  </div>
        ...
        <aside className="space-y-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              ...
              <p className="text-xs text-zinc-400">New projects auto-seed milestones; set exact dates via <b>Edit</b>. Phase aging starts today.</p>
            </CardContent>
          </Card>
        </aside>
      </main>
      ...
      <footer className="max-w-7xl mx-auto px-4 pb-10 text-xs text-zinc-500">
        Risk tints • Click-to-sort • Inline value edit • AI chips • Aging & stalled • Follow-up tracking • ICS reminders • Local persistence
      </footer>
